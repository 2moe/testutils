...
mod tests {
  ...
  #[ignore]
  fn conv_targte_list_to_rs_code(list: &str) -> String {
    let replace_with_underline = |target: &str| {
      target
        .chars()
        .map(|c| match c {
          '-' | '.' => '_',
    ...
    let (variants, matches): (String, String) = list
      ...
      .filter(|s| !s.is_empty())
      .map(|target| {
        let variant = target.pipe(replace_with_underline);
        (
          fmt_compact!("  {variant},\n"),
          format!("    {variant} => \"{target}\",\n"),
        )
      })
      .unzip();
  ...
  x86_64-apple-darwin
    "#
    .pipe(conv_targte_list_to_rs_code)
    .pipe(|x| eprintln!("generated_code: {x}"))
  }
